...
public class KFirstOne {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int m = Integer.parseInt(st.nextToken()); // Число операций
        ...
        for (int i = 0; i < m; i++) {
            ...
            int a = Integer.parseInt(st.nextToken()); // i или k
            if (command == 1) {
                int value = 0;
                ...
                arrayOfA_i[a] = value;
                update(0, 0, n - 1, a, value);
            } else {
                System.out.println(findKth(0, 0, n - 1, a));
            }
        }
    ...
    public static int findKth(int v, int tl, int tr, int k) {
        if (tl == tr) {
            return tl; // Мы дошли до листа — это и есть индекс k-й единицы
        }
        int tm = (tl + tr) / 2;

        // Смотрим, сколько единиц в левом поддереве
        long leftCount = tree[2 * v + 1];

        if (k < leftCount) {
            // Идем налево, ищем ту же k-ю единицу
            return findKth(2 * v + 1, tl, tm, k);
        } else {
            // Идем направо, но ищем (k - количество_слева) единицу
            return findKth(2 * v + 2, tm + 1, tr, (int)(k - leftCount));
        }
    }
