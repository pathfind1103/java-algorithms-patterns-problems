...
public class NumberOfMinimaOnSegment {
    static long[] tree;
    static int[] countOfMin;
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (command == 1) {
                ...
                long[] result = getMin(0, 0, n - 1, a, b - 1);
    ...
    public static void initial(int start, int l, int r, long[] array) {
        ...
        long[] res = combine(tree[2 * start + 1], countOfMin[2 * start + 1], tree[2 * start + 2], countOfMin[2 * start + 2]);
        tree[start] = res[0];
        countOfMin[start] = (int) res[1];
    }

    static long[] combine(long min1, int cnt1, long min2, int cnt2) {
        if (min1 < min2) return new long[]{min1, cnt1};
        if (min2 < min1) return new long[]{min2, cnt2};
        return new long[]{min1, cnt1 + cnt2};
    }
    ...
    public static void update(int start, int l, int r, int pos, int newValue) {
        if (l == r) {
            ...
            long[] res = combine(tree[2 * start + 1], countOfMin[2 * start + 1],
                    tree[2 * start + 2], countOfMin[2 * start + 2]);
    ...
    public static long[] getMin(int v, int tl, int tr, int l, int r) {
        if (l > r) return new long[]{Long.MAX_VALUE, 0};
        if (l == tl && r == tr) return new long[]{tree[v], countOfMin[v]};

        int tm = (tl + tr ) / 2;
        long[] left = getMin(2 * v + 1, tl, tm, l, Math.min(r, tm));
        long[] right = getMin(2 * v + 2, tm + 1, tr, Math.max(l, tm + 1), r);

        return combine(left[0], (int)left[1], right[0], (int)right[1]);
    }
}
